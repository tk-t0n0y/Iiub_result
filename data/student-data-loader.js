...
async function loadData() {
    try {
        ...
        // Get the correct data URL using the config
        const dataUrl = window.IsakhaConfig 
            ? IsakhaConfig.getResourceUrl('/data/student-data.json')
            : (window.location.hostname.includes('github.io') 
                ? `/${window.location.pathname.split('/')[1]}/data/student-data.json`
