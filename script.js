...
// Credit section protection
const CREDIT_HTML = `<div class="developer-credit">
    <p>Developed by <a href="https://github.com/isakha" target="_blank">Isakha</a></p>
</div>`;

// Calculate hash of credit section
...
async function initializeApp() {
    // Verify and protect credit section
    function verifyCreditSection() {
        const creditSection = document.querySelector('.developer-credit');
        
        // Check if credit section exists and has correct content
        if (!creditSection || 
            calculateHash(creditSection.innerHTML.replace(/\s+/g, '')) !== calculateHash(CREDIT_HTML.replace(/\s+/g, ''))) {
            
            // Attempt to restore credit section
            try {
                // Store the current state
                const data = localStorage.getItem('studentData');
                const currentSearch = document.getElementById('student-id')?.value;
                
                // Clear everything and rebuild
                document.body.innerHTML = `
                    <div class="container">
                        <div class="error-message" style="text-align: center; padding: 20px; color: red;">
                            <h2>Critical Error: Site Integrity Compromised</h2>
                            <p>This website requires proper attribution to function.</p>
                        </div>
                    </div>
                `;
                
                // Prevent any further JavaScript execution
                throw new Error('Site integrity compromised');
            } catch (e) {
    ...
    // Create immutable credit section observer with integrity check
    ...
    setInterval(verifyCreditSection, 500);

    // Override innerHTML and other DOM manipulation methods
    const originalSetAttribute = Element.prototype.setAttribute;
    Element.prototype.setAttribute = function(name, value) {
        if (this.classList?.contains('developer-credit') || this.closest('.developer-credit')) {
            verifyCreditSection();
            return;
        }
    ...
    Element.prototype.removeChild = function(child) {
        if (child.classList?.contains('developer-credit') || child.closest('.developer-credit')) {
            verifyCreditSection();
            return child;
        }
    ...
    // Protect against JavaScript injection
    const originalCreateElement = document.createElement;
    document.createElement = function(tagName) {
        const element = originalCreateElement.call(document, tagName);
        if (tagName.toLowerCase() === 'script') {
            element.addEventListener('load', verifyCreditSection);
        }
        return element;
    };
